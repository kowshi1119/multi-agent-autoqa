...
export class ExplabsCriticProvider implements CriticProvider {
  ...
  constructor(apiKey: string, private readonly logger: Logger, model: string) {
    this.client = new ExplabsClient(apiKey, model);
    this.modelId = model;
  }

  async critique(input: CriticInput): Promise<CriticDecision> {
    const first = await this.client.complete(CRITIC_SYSTEM_PROMPT, formatCriticUserMessage(input));
