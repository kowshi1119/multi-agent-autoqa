...
1. **Target binding (wrong-target prevention).** `src/profiles/fingerprint.ts` hashes the raw profile, workflows and checks files. Readiness returns `target: {profileId, name, origin, environmentKind, fingerprint}`. `POST /api/runs`, `/auth-discovery`, `/workflow-discovery` and workflow save now *require* `expected: {fingerprint, origin}` (strict schemas). The server checks it after taking the run lock and **before** readiness probes or any browser starts. On a mismatch it returns 409 `TARGET_CHANGED` and nothing is sent to any application. UI: no implicit selection ("— Choose an application —"); one `selectProfile` path for user changes, reload recovery and editor saves; an in-flight readiness request is **aborted** when the selection changes, so a stale response is ignored; Start and Discover stay disabled until the prepared target matches the current selection and mode; a target line beside Start shows application · origin · environment · operation · mode; a double click is guarded, and the server lock allows only one start.
2. **Stateful read-only workflows.** Additive schema: `select` action (declared mode only), step `resultingQuery`, and completion `query` / `absent` / `inputValue` / `count` / `changedFrom` (the result set must differ from a snapshot taken at workflow start; names are kept in memory, and evidence records only counts and the verdict). Workflows also get `reset` (verified, budget-counted) and `observed` (discovery evidence). Runtime: per-assertion `{assertion, expected, observed, passed}`; an assertion failure is reset and retried **once** from a verified state → `reproduced: true|false`; a failure kind is recorded for every workflow that did not complete (application-assertion, autoqa-control, session-expired, policy, reset, cancelled, budget, precondition, not-reached, unsupported); a failed reset blocks later workflows ("starting state unknown"); the login page is checked after every step. Discovery (`src/pilot/stateful-discovery.ts`) probes up to two list pages reached from the landing page for record-detail links, same-path filter links, pagination links, GET search forms (a word from a listed record, plus an empty-result draft only if an empty-state element is observed) and `<select>` GET filters. A form is submitted only when its GET endpoint is in `allowedFormSubmitEndpoints` and its kind is allowed; otherwise it is returned under `needsConfiguration` and **not submitted**. Save-time validation applies per-kind rules: at most 2 steps, named controls, bounded fill values, exact generated URL patterns, reset and query assertions required, sort rejected.
3. **Authentication mechanism diagnosis.** `src/auth/mechanism-observer.ts` watches the application's own fetch/XHR calls to `allowedApiOrigins` (Playwright `request.allHeaders()`), excluding the declared login exchange. It writes `auth-mechanism.json` containing **only** counts and Authorization scheme names. When the run's cookie session is rejected and the app itself uses an Authorization header on that origin, the check is recorded as a **mechanism mismatch (unsupported)**, not "session expired". New opt-in `apiChecks.runSessionAuth: "observed-authorization"` reuses, in memory only, the **Bearer** value the application itself sent to that same origin in this run. It never reads browser storage, never sends to another origin, never retains Basic credentials, and writes nothing. Proven only on the synthetic fixture's bearer mode. The ledger records `sessionAuth`.
4. **QA-lead results.** `src/reporting/qa-summary.ts` writes `qa-summary.json` from the run's own artifacts: application, origin, environment and short configuration fingerprint; operation; authentication state (not required / not attempted / failed / verified); API-authentication state (not configured / not run / unsupported / session expired / executed with run session, plus the observed mechanism); workflows selected, attempted, completed, failed, blocked, cancelled, unsupported and not attempted, each with expected vs observed and whether it reproduced; findings split into application candidates (provisional, rule-based severity with the rule stated, reproduction steps, evidence refs), AutoQA/configuration failures, and unsupported; not assessed; and browser actions / HTTP check requests / model decisions / external model requests. **A run that executed nothing says "No checks were executed — this is not a QA pass."** The UI shows this panel first in Results, and 1d now shows each draft's kind, steps, assertions, reset, observation and the needs-configuration list.
5. `profiles/auth-demo.json` (synthetic only) now allows search/filter/paginate with `GET /statements`, 60 actions and 10 pages, so the local demo shows stateful drafts. The synthetic fixture gained `/statements` (search, status filter, pagination, empty state), `/statements/:id`, known-bug switches, a page-view session limit and a bearer API mode.

### Research (accessed 2026-09-25)
- Playwright 1.62 docs (playwright.dev, installed version): `Request.allHeaders()` includes cookie and security-related headers, and `headers()` does not; `locator.selectOption` accepts a value or `{label}`.
- W3C HTML-AAM: `input[type=search]` maps to role `searchbox`. WAI-ARIA 1.2: `searchbox`, `combobox`, `table`, `row` role definitions.
- **UNRESOLVED:** the exact WHATWG "mutate action URL" text for GET form submission (the fetch tool could not retrieve that section, twice). The design does not depend on it: the expected query comes from the URL **observed** after submission, and GET is never treated as proof of safety. Supporting observation only: Chromium submitted the fixture's search form as `?q=Coffee&status=` (the form's empty "All" option is included).

### Verification
- **Synthetic UI demo** (real server build, `auth-demo` on the synthetic fixture, profiles copied to a temp dir, mock providers, 0 external model requests): fresh load → nothing selected, Start disabled; target line shown after selection; discovery drafts NAV-STATEMENTS/PROFILE/HELP, DETAIL-STATEMENTS, FILTER-STATEMENTS-PAID-ONLY, PAGE-STATEMENTS-NEXT, SEARCH-STATEMENTS, SEARCH-EMPTY-STATEMENTS, FILTER-STATEMENTS-STATUS; Delete account / Send money / Log out skipped unopened; six saved.
  - Run `RUN-20260925-061450275Z-abac`: 6/6 workflows completed; API checks executed with the run's cookie session; 18 browser actions, 4 HTTP check requests, 7 mock decisions. Verdict *needs review*, because of the demo's declared AUTH-HEADERS expectation (the synthetic fixture sends no CSP / X-Frame-Options / HSTS; unrated). AUTH-TRANSFER-BLOCKED was not sent (0 hits).
  - Repeat `RUN-20260925-061457028Z-b9fb`: identical outcome.
  - Stop `RUN-20260925-061503643Z-0593`: cancelled during login, six workflows recorded as cancelled, verdict *no checks executed*, Start usable.
- **Canonical fixture** `RUN-20260925-061537203Z-0db1`: 74 actions, 38 mock decisions, 33.9 s; detection P 0.667 / R 1.000 / F1 0.800 (6 TP, 3 FP, 0 FN); final 0.750 / 1.000 / 0.857; grouped 1.0 / 1.0 / 1.0. Benchmark definitions unchanged.
- **Intermediate** full `npm run verify:local` during development: 742 tests / 91 files passed. After that, the only source changes were `profiles/auth-demo.json` and the 1d description text, both covered by the final run.
- **Final** (frozen source): `npm run verify:local`, 2026-09-25: typecheck and build passed; **742 tests / 91 files passed, zero failures (410.13 s, two workers)** (709 baseline + 33 new); challenge corpus VALID. Source was unchanged during and after the run (identical diff hash before and after); only PROGRESS.md, README.md and docs/AJEER_PILOT_SETUP.md were edited afterwards. None of this is real-model evaluation or Ajeer acceptance.
- Failures seen during development, all fixed before freezing: an import cycle; a stale readiness request that still reached the abandoned target (now aborted); session expiry missed during the move to a workflow's start; empty evidence for unexecuted workflows; a navigate draft that asserted on a link; unbudgeted reset navigations; bearer-mode cookie rejection mislabelled as "session expired" (now a mechanism mismatch). A demo-script setup error (built UI served without copying `index.html`) was mine, not a product defect.

### New tests (33 in 6 new files, plus updates)
`tests/server/target-binding.test.ts` (5), `tests/server/target-selection-ui.test.ts` (5), `tests/pilot/stateful-workflows.test.ts` (12), `tests/checks/auth-mechanism.test.ts` (4), `tests/reporting/qa-summary.test.ts` (6), `tests/server/workflow-discovery-ui.test.ts` (1). Updated: e2e-fixture, auth-discovery-ui (the auth-only verdict must say "not a QA pass"), checks-ui, phase5, workflow-discovery. The scenarios include: rejected or stale operations sending **zero** requests to the unintended application; delayed responses with rapid switching; reloads; a configuration change between readiness and Start; double starts; stale controls; search, filter and pagination assertions; empty results; failed resets; session expiry mid-workflow; cancellation during discovery and during execution; unauthorized form endpoints never submitted; budget exhaustion; token, cookie and password absence from every artifact.

### Status matrix
| Capability | State |
|---|---|
| Target binding (UI + server, stale/race protection) | Implemented, verified locally (synthetic) |
| Search / filter / pagination / detail / empty-result workflows with reset and reproduction | Implemented, verified locally (synthetic); GET forms and links only |
| Auth-mechanism diagnosis; observed-Bearer reuse (opt-in) | Implemented, verified locally (synthetic bearer mode); **not verified against Ajeer** |
| QA summary (no-execution ≠ pass) | Implemented, verified locally |
| Ajeer authentication-only | Verified against Ajeer once (`RUN-20260923-075117245Z-df87`) |
| Ajeer workflows, stateful workflows, API checks | **Pending local sign-in by the user** |
| POST search, JS-only search, custom dropdowns, sorting, MFA/SSO | Unsupported |

**Limitations.** Stateful discovery covers GET forms and links on at most two list pages. Assertions are structural (query, result-set change, identity heading, empty-state element); record counts, balances, business rules and permissions are never inferred. Where such assertions are needed they require user configuration. Severities are provisional. The mock Explorer/Critic is not real-model evaluation.

**Next steps.** Ajeer local acceptance (docs/AJEER_PILOT_SETUP.md): authentication-only smoke, 1d discovery, one-workflow run, remaining workflows, a fresh repeat, and a Stop. Read `auth-mechanism.json` from those runs before deciding anything about Ajeer API checks.
