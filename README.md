...
Run **npm run ui**, choose **AutoQA Local Fixture → Demo → Start**. Setup runs automatically; no API key or local model is needed. Results identify the run/environment and history names the project. Stop waits for cleanup and report saving before Start becomes usable again.

For Ajeer, use **Discover** with credentials entered only in the local form. Select a stable, non-personal verified heading, save the observed conditions, then enter credentials again under **Sign in for this run** and run **Authentication only**. Discovery is not acceptance. Rediscovery is needed because an earlier banner-text suggestion could match no accessible element. Same-route login, MFA, CAPTCHA and interactive SSO are unsupported by this discovery flow. Ajeer workflows and successful authenticated acceptance remain pending.

**npm run verify:local** runs typecheck, build, the complete suite with two workers and corpus validation, excluding local credential variables and .env loading. Requires only already-installed dependencies/Chromium, no internet, API keys or models. Latest full result: **642 tests / 76 files passed**; 20 corpus cases validated. After final UI labels and a stricter origin assertion, all 11 targeted tests passed.
